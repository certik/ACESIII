...
C  in the file COPYRIGHT.
      subroutine tester_MRT4(a1,a2,b1,b2,c1,c2,d1,d2,
...
c---------------------------------------------------------------------------
      ...
      integer der_save(12)
      integer der_test(12)
      integer der_test1(12)
      integer der_test2(12)
      ...
      integer ihess, jhess, icomponent, jcomponent, kcomponent, 
     *        lhess, lcomponent   
      integer iatom, jatom, katom, iflag, jflag, kflag, 
     *        latom, lflag     
...
c--------------------------------------------------------------------------
               ...
               do katom = 1, ncenters 
                  ...
                  if ((katom .ne. atom(m)) .and. (katom .ne. atom(n))
     *            .and. (katom .ne. atom(r)) .and. (katom .ne. atom(s)))
     *            go to 666  

               do kcomponent = 1, 3 

                  do jflag = 1, 12
                     der_test1(jflag)  = der_test(jflag)
                  enddo 
...
c-------------------------------------------------------------------------
c
             if (kcomponent .eq. 1) then

                if (atom(m) .eq. katom) then

                   der_test1(1) = der_test1(1) + 1

                endif

                if (atom(n) .eq. katom) then

                   der_test1(4) = der_test1(4) + 1

                endif

                if (atom(r) .eq. katom) then

                   der_test1(7) = der_test1(7) + 1

                endif

                if (atom(s) .eq. katom) then

                   der_test1(10) = der_test1(10) + 1 
...
c-------------------------------------------------------------------------
c
             if (kcomponent .eq. 2) then

                if (atom(m) .eq. katom) then

                   der_test1(2) = der_test1(2) + 1

                endif

                if (atom(n) .eq. katom) then

                   der_test1(5) = der_test1(5) + 1

                endif

                if (atom(r) .eq. katom) then

                   der_test1(8) = der_test1(8) + 1

                endif

                if (atom(s) .eq. katom) then

                   der_test1(11) = der_test1(11) + 1
...
c-------------------------------------------------------------------------
c
             if (kcomponent .eq. 3) then

                if (atom(m) .eq. katom) then

                   der_test1(3) = der_test1(3) + 1

                endif

                if (atom(n) .eq. katom) then

                   der_test1(6) = der_test1(6) + 1

                endif

                if (atom(r) .eq. katom) then

                   der_test1(9) = der_test1(9) + 1 

                endif

                if (atom(s) .eq. katom) then

                   der_test1(12) = der_test1(12) + 1
...
c----------------------------------------------------------------------------
               ...
               if (atom(m) .eq. atom(n)) then
                  if (der_test1(1) .ne. 0) der_test1(4) = der_test1(1)
                  if (der_test1(4) .ne. 0) der_test1(1) = der_test1(4)
                  if (der_test1(2) .ne. 0) der_test1(5) = der_test1(2)
                  if (der_test1(5) .ne. 0) der_test1(2) = der_test1(5)
                  if (der_test1(3) .ne. 0) der_test1(6) = der_test1(3)
                  if (der_test1(6) .ne. 0) der_test1(3) = der_test1(6)
               endif

               if (atom(m) .eq. atom(r)) then
                  if (der_test1(1) .ne. 0) der_test1(7) = der_test1(1)
                  if (der_test1(7) .ne. 0) der_test1(1) = der_test1(7)
                  if (der_test1(2) .ne. 0) der_test1(8) = der_test1(2)
                  if (der_test1(8) .ne. 0) der_test1(2) = der_test1(8)
                  if (der_test1(3) .ne. 0) der_test1(9) = der_test1(3)
                  if (der_test1(9) .ne. 0) der_test1(3) = der_test1(9)
               endif

               if (atom(m) .eq. atom(s)) then
                  if (der_test1(1) .ne. 0) der_test1(10) = der_test1(1)
                  if (der_test1(10).ne. 0) der_test1(1)  = der_test1(10)
                  if (der_test1(2) .ne. 0) der_test1(11) = der_test1(2)
                  if (der_test1(11).ne. 0) der_test1(2)  = der_test1(11)
                  if (der_test1(3 ).ne. 0) der_test1(12) = der_test1(3)
                  if (der_test1(12).ne. 0) der_test1(3)  = der_test1(12)
               endif

               if (atom(n) .eq. atom(r)) then
                  if (der_test1(4) .ne. 0) der_test1(7) = der_test1(4)
                  if (der_test1(7) .ne. 0) der_test1(4) = der_test1(7)
                  if (der_test1(5) .ne. 0) der_test1(8) = der_test1(5)
                  if (der_test1(8) .ne. 0) der_test1(5) = der_test1(8)
                  if (der_test1(6) .ne. 0) der_test1(9) = der_test1(6)
                  if (der_test1(9) .ne. 0) der_test1(6) = der_test1(9)
               endif

               if (atom(n) .eq. atom(s)) then
                  if (der_test1(4)  .ne. 0) der_test1(10) =der_test1(4)
                  if (der_test1(10) .ne. 0) der_test1(4)  =der_test1(10)
                  if (der_test1(5)  .ne. 0) der_test1(11) =der_test1(5)
                  if (der_test1(11) .ne. 0) der_test1(5)  =der_test1(11)
                  if (der_test1(6)  .ne. 0) der_test1(12) =der_test1(6)
                  if (der_test(12)  .ne. 0) der_test1(6)  =der_test1(12)
               endif

               if (atom(r) .eq. atom(s)) then
                  if (der_test1(7)  .ne. 0) der_test1(10) =der_test1(7)
                  if (der_test1(10) .ne. 0) der_test1(7)  =der_test1(10)
                  if (der_test1(8)  .ne. 0) der_test1(11) =der_test1(8)
                  if (der_test1(11) .ne. 0) der_test1(8)  =der_test1(11)
                  if (der_test1(9)  .ne. 0) der_test1(12) =der_test1(9)
                  if (der_test1(12) .ne. 0) der_test1(9)  =der_test1(12)
               endif
...
c--------------------------------------------------------------------------
               ...
               do latom = 1, ncenters 
                  ...
                  if ((latom .ne. atom(m)) .and. (latom .ne. atom(n))
     *            .and. (latom .ne. atom(r)) .and. (latom .ne. atom(s)))
     *            go to 888  

               do lcomponent = 1, 3 

                  do jflag = 1, 12
                     der_test2(jflag)  = der_test1(jflag)
                  enddo 
...
c
             if (lcomponent .eq. 1) then
                ...
                if (atom(m) .eq. latom) then

                   x1p = x1 + eps
                   der_test2(1) = der_test2(1) + 1

                endif

                if (atom(n) .eq. latom) then

                   x2p = x2 + eps
                   der_test2(4) = der_test2(4) + 1

                endif

                if (atom(r) .eq. latom) then

                   x3p = x3 + eps
                   der_test2(7) = der_test2(7) + 1

                endif

                if (atom(s) .eq. latom) then

                   x4p = x4 + eps
                   der_test2(10) = der_test2(10) + 1 
...
c----------------------------------------------------------------------------
               ...
               if (atom(m) .eq. atom(n)) then
                  if (der_test2(1) .ne. 0) der_test2(4) = der_test2(1)
                  if (der_test2(4) .ne. 0) der_test2(1) = der_test2(4)
                  if (der_test2(2) .ne. 0) der_test2(5) = der_test2(2)
                  if (der_test2(5) .ne. 0) der_test2(2) = der_test2(5)
                  if (der_test2(3) .ne. 0) der_test2(6) = der_test2(3)
                  if (der_test2(6) .ne. 0) der_test2(3) = der_test2(6)
               endif

               if (atom(m) .eq. atom(r)) then
                  if (der_test2(1) .ne. 0) der_test2(7) = der_test2(1)
                  if (der_test2(7) .ne. 0) der_test2(1) = der_test2(7)
                  if (der_test2(2) .ne. 0) der_test2(8) = der_test2(2)
                  if (der_test2(8) .ne. 0) der_test2(2) = der_test2(8)
                  if (der_test2(3) .ne. 0) der_test2(9) = der_test2(3)
                  if (der_test2(9) .ne. 0) der_test2(3) = der_test2(9)
               endif

               if (atom(m) .eq. atom(s)) then
                  if (der_test2(1) .ne. 0) der_test2(10) = der_test2(1)
                  if (der_test2(10).ne. 0) der_test2(1)  = der_test2(10)
                  if (der_test2(2) .ne. 0) der_test2(11) = der_test2(2)
                  if (der_test2(11).ne. 0) der_test2(2)  = der_test2(11)
                  if (der_test2(3 ).ne. 0) der_test2(12) = der_test2(3)
                  if (der_test2(12).ne. 0) der_test2(3)  = der_test2(12)
               endif

               if (atom(n) .eq. atom(r)) then
                  if (der_test2(4) .ne. 0) der_test2(7) = der_test2(4)
                  if (der_test2(7) .ne. 0) der_test2(4) = der_test2(7)
                  if (der_test2(5) .ne. 0) der_test2(8) = der_test2(5)
                  if (der_test2(8) .ne. 0) der_test2(5) = der_test2(8)
                  if (der_test2(6) .ne. 0) der_test2(9) = der_test2(6)
                  if (der_test2(9) .ne. 0) der_test2(6) = der_test2(9)
               endif

               if (atom(n) .eq. atom(s)) then
                  if (der_test2(4)  .ne. 0) der_test2(10) =der_test2(4)
                  if (der_test2(10) .ne. 0) der_test2(4)  =der_test2(10)
                  if (der_test2(5)  .ne. 0) der_test2(11) =der_test2(5)
                  if (der_test2(11) .ne. 0) der_test2(5)  =der_test2(11)
                  if (der_test2(6)  .ne. 0) der_test2(12) =der_test2(6)
                  if (der_test2(12) .ne. 0) der_test2(6)  =der_test2(12)
               endif

               if (atom(r) .eq. atom(s)) then
                  if (der_test2(7)  .ne. 0) der_test2(10) =der_test2(7)
                  if (der_test2(10) .ne. 0) der_test2(7)  =der_test2(10)
                  if (der_test2(8)  .ne. 0) der_test2(11) =der_test2(8)
                  if (der_test2(11) .ne. 0) der_test2(8)  =der_test2(11)
                  if (der_test2(9)  .ne. 0) der_test2(12) =der_test2(9)
                  if (der_test2(12) .ne. 0) der_test2(9)  =der_test2(12)
               endif
...
c --------------------------------------------------------------------------
c   Calculate the unperturbed third-derivative. 
c --------------------------------------------------------------------------
...
c --------------------------------------------------------------------------
c   Calculate the perturbed third-derivative. 
c --------------------------------------------------------------------------
...
c              write(6,*) '       ATOM :', iatom,jatom,katom 
c              write(6,*) '       COMP :', icomponent, jcomponent,
c    *                                     kcomponent
c              write(6,*) '       FLAGS:', (der_test2(a),a=1,12)
...
c              go to 987 
     ...
     *            0.000001d0) then 
               ...
               write(6,*) ' MISMATCH IN FOURTH DERIVATIVE' 
               write(6,*) 'SATOM :', atom(m),atom(n),atom(r),atom(s)
               write(6,*) ' ATOM :', iatom,jatom,katom,latom 
               write(6,*) ' COMP :', icomponent, jcomponent,kcomponent
     *                             , lcomponent 
               write(6,*) '       FLAGS:', (der_test2(a),a=1,12)
...
987            continue 
            ...
            enddo ! latom = 1, ncenters 
888         continue
            enddo ! lcomponent = 1, 3 

            enddo ! katom = 1, ncenters 
666         continue
            enddo ! kcomponent = 1, 3 
