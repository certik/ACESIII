...
C  in the file COPYRIGHT.
      subroutine compute_abcd_integrals(a1,a2,b1,b2,c1,c2,d1,d2,scr,
...
c-----------------------------------------------------------------------
         ...
         do n = n1, n2
            ...
            z2 = coords(3,n)
         do r = r1, r2
            if (atom(r) .eq. atom(m)) go to 10 
            if (atom(r) .eq. atom(n)) go to 10 
            if (r .eq. 1) then
               cc1 = 1
            else
               cc1 = end_nfps(r-1) + 1
            ...
            z3 = coords(3,r)
         do s = s1, s2
            if (atom(s) .eq. atom(m)) go to 30 
            if (atom(s) .eq. atom(n)) go to 30 
            if (atom(s) .eq. atom(r)) go to 30 
               x4 = coords(1,s)
               y4 = coords(2,s)
               z4 = coords(3,s)
